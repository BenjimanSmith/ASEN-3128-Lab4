...
conditions(7) = deg2rad(5); % bank angle is 5 degrees
...
[t1, X] = ode45(@(t, F)Specs2LB4LC(t, F, TrimForces, Pertubations, givens), tspan, conditions, options); % ode call for linearized feedback control ode
...
sgtitle('Deviation by +5^{\circ} in bank');
 subplot(4,2,1);
plot(t1, X(:,7));
...
TrimForces = ones(1, 4) * m * g / 4; % forces required by each motor to maintain hover
conditions = zeros(1, 12); % initialize conditions vector (very large)
conditions(8) = deg2rad(5); % elevation of 5 degrees
conditions(12) = -1; % set down direction to 1 to make signs correct for plotting
t1 = 0;
t2 = 0;
X = 0;
...
%% plotting
figure()
sgtitle('Deviation by +5^{\circ} in elevation'); subplot(4,2,1);
plot(t1, X(:,7));
...
TrimForces = ones(1, 4) * m * g / 4; % forces required by each motor to maintain hover
conditions = zeros(1, 12); % initialize conditions vector (very large)
conditions(9) = deg2rad(5); % azimuth angle is 5
conditions(12) = -1; % set down direction to 1 to make signs correct for plotting
t1 = 0;
t2 = 0;
X = 0;
...
figure()
sgtitle('Deviation by +5^{\circ} in azimuth'); subplot(4,2,1);
plot(t1, X(:,7));
...
TrimForces = ones(1, 4) * m * g / 4; % forces required by each motor to maintain hover
conditions = zeros(1, 12); % initialize conditions vector (very large)
conditions(4) = 0.1; % roll rate is 0.1 rad/s
conditions(12) = -1; % set down direction to 1 to make signs correct for plotting
t1 = 0;
t2 = 0;
X = 0;
X2 = 0;
options = odeset('Events', @StopFnct, 'RelTol', 1e-8); % stop function that ends ODE when a tolerance of 1e-8 is met
[t1, X] = ode45(@(t, F)Specs2LB4LC(t, F, TrimForces, Pertubations, givens), tspan, conditions, options);
...
TrimForces = ones(1, 4) * m * g / 4; % forces required by each motor to maintain hover
conditions = zeros(1, 12); % initialize conditions vector (very large)
conditions(5) = 0.1; % pitch rate is 0.1 rad/s
conditions(12) = -1; % set down direction to 1 to make signs correct for plotting
t1 = 0;
t2 = 0;
X = 0;
...
TrimForces = ones(1, 4) * m * g / 4; % forces required by each motor to maintain hover
conditions = zeros(1, 12); % initialize conditions vector (very large)
conditions(6) = 0.1; % yaw rate is 0.1 rad/s
conditions(12) = -1; % set down direction to 1 to make signs correct for plotting
t1 = 0;
t2 = 0;
X = 0;
