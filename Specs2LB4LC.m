...
%Benjiman Smith
%02/13/2020
function dydt = Specs2LB4LC(t, Conditions, Force, Disturb, givens)
    % define givens
 ...
 Lcontrol = ((f1 + f2) - (f3 + f4)) * R; % p component of the control moments
    ...
    deltapdot = ((-k_1*deltap)/I_x) -((k_2*deltaphi)/I_x);
    deltaqdot = ((-k_3*deltaq)/I_y) -((k_4*deltatheta)/I_y); % pitch rate derrivative
